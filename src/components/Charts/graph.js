...
import DATASET from '../data/patientData.js';
...
for (var i = 0; i < data.length; i++) {
  if(regions.hasOwnProperty(data[i].region))
      ...
      regions[data[i].region] = [0, 1, `rgba(${r}, ${g}, ${b}, 0.6)`, `rgba(${r}, ${g}, ${b}, 1)`];     // [recovered, total, bgcolor, bordercolor]
  }
  if(data[i].status === 'Recovered')
      regions[data[i].region][0]++;

      if(diseases.hasOwnProperty(data[i].disease))
      diseases[data[i].disease][2]++;
  else{
      ...
      diseases[data[i].disease] = [0, 0, 1, `rgba(${r2}, ${g2}, ${b2}, 0.6)`, `rgba(${r2}, ${g2}, ${b2}, 1)`];  // [recovering, deaths, total, bgcolor, bordercolor]
  }
  if(data[i].status === 'Recovering')
      diseases[data[i].disease][0]++;
  else if(data[i].status === 'Passed Away')
      diseases[data[i].disease][1]++;
}
